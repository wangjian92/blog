...
即方法一的源码，可部署在自己的 VPS 上进行测试，这里有一个 websocket 服务`websocket-starter`，用于启动 websocket 测试 nginx 代理是否成功，默认在 3000 端口，可在`server.js`中修改
...
进入配置文件或者配置页面，配置服务器 ip 地址、端口、uuid
